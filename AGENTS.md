...
- 每个尚未发布的开发批次只确定一个目标版本；代码审查、测试修复、文案校正和同批次返工都合并进该目标版本，不因提交次数或审查轮次重复涨号。
- 已发布版本之后，向后兼容的缺陷修复升 patch，新增向后兼容能力升 minor，不兼容变化升 major；`0.x` 阶段仍保持“patch 用于兼容修复、minor 用于新能力或不兼容变化”的稳定约定。
- 仅调整安装示例、拼写、链接或不影响已安装 Skill 行为的分发元数据时不升级 Skill 版本；若文档变化反映了真实行为、权限或安全语义变化，则按实际影响升级版本。
- 平台审核撤回、拒绝或版本占号只记录为渠道状态，不自动推动仓库 `VERSION`；若渠道要求更高包版本但仓库没有可发布变化，应先扩展渠道专属包版本机制，而不是空增规范版本。
- 已发布版本不得降级、复用或移动 Tag；后续修复从当前已发布版本继续按 SemVer 递增。
