...
record for each selected channel. It does not bypass login, legal, or final
submission controls.
...
- prepares multi-source batches with digest-bound batch confirmation scopes;
- resumes from SQLite state and records only real monitor transitions;
- refreshes `state/publishing-ledger.md` automatically;
- validates listing assets, channel field limits, and package exclusions before upload.

Packages can currently be prepared for Coze Skill Store, WorkBuddy, SkillPay,
and Xiaohongshu Red Skill. Zhihu AI Works is not available until its creator form can be observed
and verified.
